...
var layout = [{
	...
	},
	{
		...
		style: 'width: 10%',
		render: function(row) {
			return row.children != undefined ? "" : '<span class="layui-btn layui-btn-normal layui-btn-xs">已启用</span>';
		}
	},
	{
		name: '操作',
...
function parseNodes(data) {
	...
	for (var i in data['tl']) {
		typeList[data['tl'][i]['ID'] - 1] = {"id": data['tl'][i]['ID'], "name": data['tl'][i]['Name'], "children": []}
	}

	for (var i in data['l']) {
		var row = data['l'][i]
		idx = typeList[row['Type'] - 1]['children'].length
		typeList[row['Type'] - 1]['children'][idx] = {"id": row['ID'], "name": row['Name']}
	}
